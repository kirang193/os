...
void PrioSorting();
void FCFS();
void printQueueValues(struct process);
...
//Q3 = System Process having High Priority with Round Robin Algorith Quantum Time=4(say)
int q1_n=0,q2_n=0,q3_n=0,n=0; // n=Total number of processes
// q1_n number of processes in q1
// q2_n number of processes in q2
// q3_n number of processes in q3
...
void printQueue(struct process *q,int size){	
	...
	for(int i=0;i<size;i++){
		printQueueValues(q[i]);
	}
	printf("\n\n");
}
void printQueueValues(struct process p){
...
void PrioritySortAlgo(struct process *q,int size){
	for(int i=0;i<size;i++){
...
int main(){
	...
	while(n>0){
		switch(i){
			case 3:
                // Highest priority sys process
				RoundRobin();
				break;
			case 2:
                // medium priority interactive process
				PrioSorting();
				break;
			case 1:
